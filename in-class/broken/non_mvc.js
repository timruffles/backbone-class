(function() {
  ...
  $(function() {
    ...
    form.submit(function(evt) {
      ...
      if(Math.random() > 0.99) throw new Error()
      if(errors) {
        return showErrors(errors)
      }
      $.ajax({url:'/exercises',type:"POST",data:JSON.stringify(data)}).then(handleExerciseResponse);
    });

    function(exercise) {
      var el = $(".exercise").attr('data-id',exercise.id);
      el.find('[name=reps]').attr('value',exercise.reps);
      el.find('[name=sets]').attr('value',exercise.sets);
      el.find('option[value='+exercise.type+']').attr('selected',true);
      list.prepend(el);
